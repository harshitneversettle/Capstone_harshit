...
describe("LFG!!", () => {
  ...
  await connection.requestAirdrop(HARSHIT_KEYPAIR.publicKey, 100 * LAMPORTS_PER_SOL)
);

await connection.confirmTransaction(
  await connection.requestAirdrop(TEST_KEYPAIR.publicKey, 100 * LAMPORTS_PER_SOL)
);
  }) ;
  ...
  it("Initializing treasury -> " , async () => {
    [treasuryStatePda, treasuryBump] = PublicKey.findProgramAddressSync(    // only findProgramAddress is a async function , toh we are using findProgramAddressSync
  ...
  throw new Error("Treasury not initialized");
    ...
    console.log("User WSOL balance before wrapping:", Number(beforeBalance)/ 1e9);
  ...
  it("Lending process begins here -> Pool initialization -> Collateral deposit ->  borrow amount", async () => {
    ...
    console.log("Harshit wallet:", 
      (await connection.getBalance(HARSHIT_KEYPAIR.publicKey)) / LAMPORTS_PER_SOL, "SOL"
    );
    console.log("Test wallet:", 
      (await connection.getBalance(TEST_KEYPAIR.publicKey)) / LAMPORTS_PER_SOL, "SOL"
    );
    console.log("-----------------------------------------------------");
    ...
    try{
      ...
      throw new Error("Run initializeTreasury first. it is not initialized yet");
    }
    ...
    collateralMint = await createMint(
      ...
      vaultAta = getAssociatedTokenAddressSync(collateralMint, vaultAuthority, true);
   ...
   console.log("User collateral ATA:", userCollateralAta.address.toBase58());
    ...
    const tx2 = await program.methods.depositCollateral(new anchor.BN(amount)).accounts({
      poolState: poolPda,
      vaultAuthority,
      collateralMint,
      vaultAta,
    ...
    console.log("\n=== Deposit Results ===");
    console.log("Pool collateral recorded:", pool.collateralAmount.toNumber(), "tokens");
    console.log("Vault actual balance:", Number(afterBal), "tokens");
    console.log("User remaining balance:", Number(beforeBal) - amount, "tokens");
    console.log("-----------------------------------------------------");
  });
  ...
  it("Borrowing from treasury-> " , async () => {
    ...
    const tx2 = await program.methods.borrowLoan().accounts({
    ...
    console.log("  Collateral Amount:", poolState.collateralAmount.toNumber(), "tokens");
    console.log("  Loan Mint:", poolState.loanMint.toBase58());
    console.log("  Loan Amount:", poolState.loanAmount.toNumber() / LAMPORTS_PER_SOL, "SOL");
    console.log("  Vault ATA:", poolState.vaultAta.toBase58());
    console.log("  Interest Rate:", poolState.interestRate.toNumber() / 100, "%");
    ...
    const treasuryLiq = (treasuryState.totalLiquidity.toNumber() / LAMPORTS_PER_SOL).toFixed(2);
    const totalBorrowed = (treasuryState.totalBorrowed.toNumber() / LAMPORTS_PER_SOL).toFixed(2);
    const userCollateral = poolState.collateralAmount.toNumber();
    const userLoan = (poolState.loanAmount.toNumber() / LAMPORTS_PER_SOL).toFixed(2);

    console.log("┌───────────────────────┬─────────────────┐");
    console.log(`│ Treasury Liquidity    │ ${treasuryLiq.padStart(9)} SOL   │`);
    console.log(`│ Total Borrowed        │ ${totalBorrowed.padStart(9)} SOL   │`);
    console.log(`│ User Collateral       │ ${userCollateral.toString().padStart(7)} tokens  │`);
